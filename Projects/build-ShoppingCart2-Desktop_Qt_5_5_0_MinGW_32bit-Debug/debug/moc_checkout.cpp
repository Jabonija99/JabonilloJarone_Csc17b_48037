/****************************************************************************
** Meta object code from reading C++ file 'checkout.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../ShoppingCart2/checkout.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'checkout.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Checkout_t {
    QByteArrayData data[8];
    char stringdata0[54];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Checkout_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Checkout_t qt_meta_stringdata_Checkout = {
    {
QT_MOC_LITERAL(0, 0, 8), // "Checkout"
QT_MOC_LITERAL(1, 9, 8), // "toFinish"
QT_MOC_LITERAL(2, 18, 0), // ""
QT_MOC_LITERAL(3, 19, 6), // "toShop"
QT_MOC_LITERAL(4, 26, 6), // "toCart"
QT_MOC_LITERAL(5, 33, 6), // "goNext"
QT_MOC_LITERAL(6, 40, 6), // "goShop"
QT_MOC_LITERAL(7, 47, 6) // "goBack"

    },
    "Checkout\0toFinish\0\0toShop\0toCart\0"
    "goNext\0goShop\0goBack"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Checkout[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x06 /* Public */,
       3,    0,   45,    2, 0x06 /* Public */,
       4,    0,   46,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    0,   47,    2, 0x0a /* Public */,
       6,    0,   48,    2, 0x0a /* Public */,
       7,    0,   49,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void Checkout::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Checkout *_t = static_cast<Checkout *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->toFinish(); break;
        case 1: _t->toShop(); break;
        case 2: _t->toCart(); break;
        case 3: _t->goNext(); break;
        case 4: _t->goShop(); break;
        case 5: _t->goBack(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Checkout::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Checkout::toFinish)) {
                *result = 0;
            }
        }
        {
            typedef void (Checkout::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Checkout::toShop)) {
                *result = 1;
            }
        }
        {
            typedef void (Checkout::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Checkout::toCart)) {
                *result = 2;
            }
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject Checkout::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Checkout.data,
      qt_meta_data_Checkout,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *Checkout::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Checkout::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_Checkout.stringdata0))
        return static_cast<void*>(const_cast< Checkout*>(this));
    return QDialog::qt_metacast(_clname);
}

int Checkout::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void Checkout::toFinish()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void Checkout::toShop()
{
    QMetaObject::activate(this, &staticMetaObject, 1, Q_NULLPTR);
}

// SIGNAL 2
void Checkout::toCart()
{
    QMetaObject::activate(this, &staticMetaObject, 2, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
